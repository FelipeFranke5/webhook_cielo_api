...
public class NotificationServiceTest {
    ...
    void getNotificationByPaymentId_ValidPaymentIdNotFound_ShouldThrowNotificationNotFoundException() {
        UUID id = UUID.randomUUID();
        String paymentIdUserEntered = id.toString();
    ...
    @Test
    @DisplayName("Method 'isNotificationSaved' - Fail to Save/notificationRepository.save - Should return false")
    void isNotificationSaved_NotificationFails1_ShouldReturnFalse() {
        String theID = UUID.randomUUID().toString();
        NotificationRequestDTO notificationRequest = new NotificationRequestDTO(theID, null, 1);
        Notification notification = Notification.fromRequest(notificationRequest);
        Mockito.when(notificationRepository.save(notification)).thenThrow(IllegalArgumentException.class);
        boolean result = notificationService.isNotificationSaved(notificationRequest);
    ...
    @Test
    @DisplayName("Method 'isNotificationSaved' - Fail to SendKafka/kafkaTemplate.send - Should return false")
    void isNotificationSaved_NotificationFails2_ShouldReturnFalse() {
        String theID = UUID.randomUUID().toString();
        NotificationRequestDTO notificationRequest = new NotificationRequestDTO(theID, null, 1);
        Notification notification = Notification.fromRequest(notificationRequest);
        Mockito.when(kafkaTemplate.send("webhook-cielo", "paymentId", theID)).thenAnswer(invocation -> {
            throw new Exception();
        });
        boolean result = notificationService.isNotificationSaved(notificationRequest);
        Assertions.assertFalse(result);
    ...
    @Test
    @DisplayName("Method 'isNotificationSaved' - Invalid PaymentId - Should throw InvalidNotificationException")
    void isNotificationSaved_NotificationFails3_ShouldThrow() {
        String theID = "invalid";
        NotificationRequestDTO notificationRequest = new NotificationRequestDTO(theID, null, 1);
        InvalidNotificationException exception = Assertions.assertThrows(InvalidNotificationException.class, () -> {
            notificationService.isNotificationSaved(notificationRequest);
        });
        Assertions.assertEquals("Invalid payload: PaymentId/RecurrentPaymentId should be UUID", exception.getMessage());
        Mockito.verify(kafkaTemplate, Mockito.never()).send("webhook-cielo", "paymentId", theID);
    }
    ...
    void isNotificationSaved_NotificationFails4_ShouldThrow() {
        ...
        NotificationRequestDTO notificationRequest = new NotificationRequestDTO(theID, invalidRecurrentPaymentId, 1);
        InvalidNotificationException exception = Assertions.assertThrows(InvalidNotificationException.class, () -> {
            notificationService.isNotificationSaved(notificationRequest);
        });
        Assertions.assertEquals("Invalid payload: PaymentId/RecurrentPaymentId should be UUID", exception.getMessage());
    ...
    @Test
    @DisplayName("Method 'isNotificationSaved' - PaymentId null - Should throw InvalidNotificationException")
